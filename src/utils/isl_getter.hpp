...
class ISL_Getter {
  ...
  clean_lib_names(const std::vector<std::string> &libs);

  void process_cmake(std::string git_file_path, std::string lib_file_path,
                     bool already_built);

  void process_header(std::string git_file_path, std::string lib_file_path,
                      bool already_built);
